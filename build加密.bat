...
set TT_CYINC=*.py controller/base_api.py service/*.py service/**/*.py fsm/*.py fsm/**/*.py
rem 在以上TT_CYINC包括的文件中，需要排除的文件列表。同样支持多个glob通配符
set TT_CYEXC=Python38-build setup.py *_call.py expr_core.py service/*/__init__.py service/*_call.py eval_expr.py
...
if defined TT_BUILDALL (
    ...
    echo [%time%] Git下载源码
    git clone --single-branch %TT_GIT% target

    echo [%time%] Pip安装第三方模块
    python -m pip install -r target\requirements.txt -i https://pypi.doubanio.com/simple/

    set TT_VE_PATH=ve

    echo [%time%] 下载Python运行时并解压缩
    git clone -b build --single-branch %TT_GIT% res
    powershell Expand-Archive -Force res\runtime.zip target\runtime

) else (
    echo [%time%] 复制源码
    robocopy .. target /mir /xd build /xd %TT_VE_PATH% /xd %TT_RT_PATH% /xd %TT_CLR_DIRS% /xf %TT_CLR_FILES% /nfl /ndl

    echo [%time%] 复制Python运行时
    robocopy %TT_RT_PATH% target\runtime /mir /nfl /ndl
)

echo [%time%] 复制第三方模块到构建目录
robocopy %TT_VE_PATH%\Lib\site-packages target\site-packages /mir /xd __pycache__ pip* setuptools* Cython* /ndl /nfl
...
if defined TT_BUILDALL (
    echo [%time%] 创建Zip发布包
    powershell Compress-Archive -Path target\* -DestinationPath release.zip
